...
fn search_returns_ranked_results_with_source_and_score() {
    ...
    let indexed = lookup.indexed_model_version().unwrap().expect("index has a model");
    assert_eq!(indexed.dimension, DIM);
    let store = RuVectorSearchAdapter::open(&paths.vectors(), indexed.dimension).unwrap();
    let svc = SearchService { embedder: &MockEmbedder::new(DIM), store: &store, lookup: &lookup };

    let run = svc.search(&query("architecture boundaries", MetadataFilter::default()), false, "rrun_it", NOW).unwrap();
    ...
    for r in &run.results {
        assert!(r.source_path.ends_with(".md"), "source present: {}", r.source_path);
        assert!((0.0..=1.0).contains(&r.score), "score in range: {}", r.score);
    }
    // scores are non-increasing by rank  (AC-2)
    for w in run.results.windows(2) {
        assert!(w[0].score >= w[1].score, "scores non-increasing: {} then {}", w[0].score, w[1].score);
    }
...
fn project_and_tag_and_source_filters_apply() {
    ...
    let all = svc.search(&query("rules", MetadataFilter::default()), false, "r", NOW).unwrap();
    assert!(all.results.len() >= 2, "both projects' chunks should be searchable");
    ...
    assert!(only_flexid.results.iter().all(|r| r.source_path.ends_with("arch.md")));

    // tag filter → only ops
    let f = MetadataFilter { tags: vec!["ops".into()], ..Default::default() };
    let only_ops = svc.search(&query("rules", f), false, "r", NOW).unwrap();
    assert!(!only_ops.results.is_empty());
    assert!(only_ops.results.iter().all(|r| r.source_path.ends_with("ops.md")));

    // source filter → exact file
    let src = only_flexid.results[0].source_path.clone();
    let f = MetadataFilter { source: Some(src.clone()), ..Default::default() };
    ...
    let f = MetadataFilter { project: Some("nonexistent".into()), ..Default::default() };
...
fn empty_index_reports_index_empty() {
    ...
    let paths = Paths::new();
    let lookup = RedbDocumentLookup::open(&paths.documents()).unwrap();
    assert!(lookup.indexed_model_version().unwrap().is_none(), "no model on an empty index");
}
